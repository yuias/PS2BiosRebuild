...
| `IGREETING` | once, then freed | `src/iop/igreeting.cpp` |
...
**`IGREETING` is the only module here that asks to be freed, and a
bump-placing loader honours that by not advancing.** IRX-13's one-shot shape
was expressible but unused: every other module on the list stays, so BOOT-8e's
residency bits had nothing to act on and the loader ignored them. It now
leaves the placement cursor where it was, and the next module is placed over
the freed one -- `SIFCMD` moves down by `0x1E0`, exactly `IGREETING`'s image,
when its entry returns 1 rather than 0. The reference releases the block to
`sysmem` instead, which for a list walked in order is the same outcome, since
its heap top is where the next module goes either way. `tools/iopsim.py`'s
IRX-12a count stays at zero for us regardless: it counts `sysmem` ordinal 5
calls, and the boot list's modules never reach that ordinal here.

The banner text is ours, not the reference's (`docs/clean-room-policy.md` §3).
What is reproduced is BOOT-8b's behaviour -- the line is chosen from boot

...
`EESYNC` is the first module compiled this way. `ps2AddModule` pins compiled
IOP code to `-march=mips1 -mno-check-zero-division`: the toolchain's default is
MIPS III, which the EE has and the R3000A does not, and at MIPS III a 64-bit
type emits `daddu`/`sd` and a division emits `teq` — none of which the IOP
would execute. What stays in assembly on the IOP is what IRX-8 specifies as
exact instruction words, the import and export stubs; `SYSMEM` and `LOADCORE`
are assembly still and are rewritten as they are next touched.

...
| The memory card (IOP-15) | **built and gated on both emulators** — a card driver that detects, authenticates, mounts and reads pages, a service over it, and a shell entry that lists the root directory. Read only: no write, create, delete or format, and no support for the older card format. Checked against two real card images whose contents are known independently |

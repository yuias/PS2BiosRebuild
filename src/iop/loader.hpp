...
// BOOT-8c: a module's base is its record plus this, in both reference loaders.
inline constexpr uint32_t kRecordSize = sizeof(ModuleRecord);
...
[[nodiscard]] static Loaded placeModule(uint32_t rom_address,
    ...
    // BOOT-8c: a module's record is the 0x30 bytes *below* its base, which is
    // what makes a load address and a record address the same number plus a
    // constant -- the reference's loaders both place a module at
    // `record + 0x30` and nothing has to carry the two separately.
    const uint32_t base_address = record_address + sizeof(ModuleRecord);
    const Placed placed = place(rom, segments, base_address);

    auto *record = reinterpret_cast<uint8_t *>(record_address);
    for (uint32_t at = 0; at < sizeof(ModuleRecord); at += 4) {
        poke32(record + at, 0);
    }
    // IRX-12c: the fields a loader here actually fills. The name and version
    // are left zero: nothing reads them, and inventing a pointer into the
    // module would be worse than a hole a reader can see.
    ...
    loaded.gp = placed.gp;
    loaded.next_base = (placed.end + 15) & ~uint32_t{15};   // the next record
    return loaded;
}
